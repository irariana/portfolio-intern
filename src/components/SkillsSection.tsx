...
import { Code, Database, BarChart3, GitBranch, Calculator, LineChart, PieChart, Brain, Wrench } from "lucide-react";
...
function ProgressBar({
  ...
  "h-2 rounded-full", "bg-muted", "overflow-hidden")}>
      ...
      <div className={cn("h-full rounded-full", "gradient-primary",
...
function SkillCard({
  ...
  return <div className={cn("p-4 rounded-lg", "glass", "group", "hover:border-primary/30", "transition-all duration-300",
  // Animation d'apparition
  animated && "animate-fade-in")} style={{
    animationDelay: `${index * 50}ms`
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <IconComponent className={cn("w-5 h-5 text-primary", "group-hover:scale-110 transition-transform")} />
      ...
      <ProgressBar value={skill.level} animated={animated} delay={index * 100 + 200} />
    </div>;
}
...
export function SkillsSection() {
  ...
  return <section ref={sectionRef} id="skills" className="py-20 md:py-32 relative">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-16">
          {/* Badge "Compétences" */}
          <div className={cn("inline-flex items-center gap-2", "px-4 py-2 rounded-full", "glass text-primary text-sm font-medium", "mb-4")}>
            <Wrench className="w-4 h-4" />
            Compétences
          </div>

          {/* Titre principal */}
          <h2 className="text-3xl md:text-4xl font-display font-bold gradient-text mb-4">Mon Stack Technique</h2>
        ...
        <div className="space-y-12">
          {Object.entries(skillsByCategory).map(([category, categorySkills], categoryIndex) => <div key={category}>
                {/* Titre de la catégorie */}
                <h3 className={cn("text-xl font-display font-semibold", "text-foreground mb-6", "flex items-center gap-2")}>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {categorySkills.map((skill, skillIndex) => <SkillCard key={skill.id} skill={skill} index={categoryIndex * 3 + skillIndex} animated={isVisible} />)}
                </div>
              </div>)}
        </div>
